...
- **既知の落とし穴（2026-09-15）**: 当初のプロンプトは「写っている食品を識別」としか指示しておらず、AIが「麻婆豆腐」のように完成料理名1つにまとめて返すか、豆腐・ねぎ・ひき肉…と食材ごとに分解して返すかが写真次第でブレていた。完成料理名1つにまとまると、パントリー在庫（生の食材単位）と一致しようがなく紐づけ・減算が効かなくなる。→ プロンプトに「完成料理名1つにまとめず、必ず主要な食材ごとに分解して列挙する」ことを明示的に指示して修正
