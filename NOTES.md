...
- さらに実機のDevTools Networkタブで確認したところ、503が返るまでに1分以上かかるケースがあった（即座にエラーが返るわけではない）。放置すると「解析中…」のまま数分固まって見えるため、各試行に`AbortController`で25秒のタイムアウトを設定。タイムアウトした場合もserver_overloaded扱いにして早めにエラー表示する
